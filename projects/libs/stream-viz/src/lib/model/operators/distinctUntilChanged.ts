import { BehaviorSubject, distinctUntilChanged, filter, map, Subject, switchMap, throttleTime } from 'rxjs';
...
export class DistinctUntilChangedOperator extends Operator {
  override type = 'distinctUntilChanged';
        ...
        this.inputEmitterObservables[e.id] = {
          ...
          observable: this.value1$.pipe(
            map((filterFunctionString) => {
              ...
              try {
                var body = 'function( prev, curr ){ return ' + filterFunctionString + ' }';
            ...
            switchMap((t) =>
              source.asObservable().pipe(
                distinctUntilChanged((prev, curr) => {
                  ...
                  console.log('distinctUntilChanged', prev.value, curr.value);
